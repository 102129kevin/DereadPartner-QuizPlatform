...
AFRAME.registerComponent("scenemanager", {
  schema: {
    ...
    lonMode: { default: "basic" },
  },

  init: function () {
    //var nft1 = document.querySelector("#nft1");
    let nft2 = document.querySelector("#nft2");
    let nft2modelInfo = this.getObjectInfo(
      nft2.querySelector("a-entity[gltf-model]")
    );
    //--------------------------------testzone--------------------------------
    ...
    //--------------------------------testzone--------------------------------
    let camera = document.querySelector("#mycamera");
    ...
    nft2.addEventListener("markerFound", () => {
      //元件建立與附加至dom
      //camera
      //add hello text (for test camera render only)
      let text = document.createElement("a-text");
      text.setAttribute("id", "Hello");
      text.setAttribute("value", "Hello");
      text.setAttribute("align", "center");
      text.setAttribute("position", "0 -1 -5");
      ...
      btnEast.addEventListener(
        "click",
        this.btnEarthRotation.bind(this, nft2modelInfo, "east") //go east 30
      );
      //綁定btnWest事件
      btnWest.addEventListener(
        "click",
        this.btnEarthRotation.bind(this, nft2modelInfo, "west") //2=go west 30
      );
  ...
  refreshRotation: function (modelInfo) {
    //console.log(id);
  ...
  btnEarthRotation: function (modelInfo, direction) {
    //declaration
    let nft2model = document.querySelector("#" + modelInfo.id);
  ...
  checkLon0: function (targetBoard) {
    //alert(this.data.longitude);
  ...
  checkLon180: function (targetBoard) {
    //alert(this.data.longitude);
    let nowLon=this.data.longitude;
  ...
  refreshLonBoardInfo: function (lon) {
    //取得該經經度資料
